...
func RouteCollections(handler *Handler, r *mux.Router) {
	...
	r.HandleFunc("/search", handler.Web(handleSearchRedirect, UserLevelReader)).Methods("GET")
	r.HandleFunc("/search:{query}", handler.Web(handleViewCollectionSearch, UserLevelReader))
	r.HandleFunc("/search:{query}/page/{page:[0-9]+}", handler.Web(handleViewCollectionSearch, UserLevelReader))
